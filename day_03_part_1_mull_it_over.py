...
def extract_and_sum_multiplications(file_path):
    """
    Extract valid `mul(X,Y)` instructions from the corrupted memory
    ...
    with open(file_path, 'r') as file:
        for line in file:
            # Find all valid `mul(X,Y)` instructions in the line
            matches = valid_mul_pattern.findall(line)
            for match in matches:
                # Extract numbers X and Y
                x, y = map(int, match[4:-1].split(','))
                # Compute their product and add to the total sum
                total_sum += x * y
