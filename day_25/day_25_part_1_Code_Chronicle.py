...
def count_valid_pairs(locks, keys):
    ...
    for lock in locks:
        for key in keys:
            if all(key[c] + lock[c] <= MAX  for c in range(len(lock))):
                valid_pairs += 1
    return valid_pairs
...
def parse_schematics(schematics):
    ...
    for schematic in schematics:
        ...
        if rows[0] == '#' * len(rows[0]):  # It's a lock: the top row is filled. 
            columns = list(zip(*rows[1:]))
            lock = count_heights (columns)
            locks.append(lock)
        elif rows[-1] == '#' * len(rows[-1]): # It's a key: the bottom row filled
            columns = list(zip(*rows[:-1]))
            key = count_heights (columns)
            keys.append(key)
    return locks, keys

def main():
    schematics = open("./day_25/day_25_input.txt", "r").read().strip().split("\n\n")
    locks, keys = parse_schematics(schematics)
    valid_pairs = count_valid_pairs(locks, keys)
    print(valid_pairs)
